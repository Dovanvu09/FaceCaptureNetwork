...
client_lib = ctypes.CDLL('/home/dvai/Work_Study/FaceCaptureNetwork/libclient.so')  #đọc thư viện client

# Khai báo các phương thức từ thư viện C++
client_new = client_lib.client_new     # tạo một đối tượng client mới 
client_new.argtypes = [ctypes.c_char_p, ctypes.c_int]    # định nghĩa tham số đầu vào
client_new.restype = ctypes.POINTER(ctypes.c_void_p)     # định nghĩa kiểu dữ liệu mà hàm client new trả về

client_delete = client_lib.client_delete                # định nghĩa phương thức delete
client_delete.argtypes = [ctypes.POINTER(ctypes.c_void_p)]    #  phương thức này nhận vào một tham số con trỏ tới đối tượng cần giải phóng

client_send_image = client_lib.client_send_image       # định nghĩa phương thức gửi ảnh từ client
client_send_image.argtypes = [ctypes.POINTER(ctypes.c_void_p), ctypes.c_char_p]      # khai báo tham số của phương thức gửi ảnh

client_receive_image_and_decompress = client_lib.client_receive_image_and_decompress     # định nghĩa phương thức nhận ảnh và giải nén ảnh 
client_receive_image_and_decompress.argtypes = [ctypes.POINTER(ctypes.c_void_p), ctypes.c_char_p]    # khai báo 2 tham số cho phương thức này
  
client_send_request_to_view_images = client_lib.client_send_request_to_view_images          # định nghĩa phương thức yêu cầu xem ảnh đến server
client_send_request_to_view_images.argtypes = [ctypes.POINTER(ctypes.c_void_p), ctypes.c_char_p, ctypes.c_char_p]      # khai báo tham số
...
class Home_User(QMainWindow):
    def __init__(self):
        super().__init__()
        self.initialize_UI()
        self.server_ip = b"172.29.84.138"
...
#------------------------------------------------------------ view session worked and log out ---------------------------------------------------------
    def Review(self):
        my_client = client_new(self.server_ip, self.server_port)
        client_send_request_to_view_images(my_client,1)
...
# --------------------------------- ----------------------------other function ---------------------------------------------------
    ...
    def send_Image_to_Server(self):
        ...
        # Nếu không có ảnh nào, thông báo cho người dùng
        if not self.faces_data:
            self.show_Error("No faces detected.")
            return
        name = self.name_input.text().encode('utf-8')  # Chuyển đổi tên người dùng thành bytes
        # Khởi tạo đối tượng client
        my_client = client_new(self.server_ip, self.server_port)
        # Gửi ảnh
        try:
            # Gửi dữ liệu ảnh dưới dạng chuỗi byte
            client_send_image(my_client, b"path_to_image")
            QMessageBox.information(self, "Success", "Image sent to the server.", QMessageBox.StandardButton.Ok)
        except Exception as e:
            self.show_Error(f"Failed to send image: {str(e)}")
        finally:
            # Giải phóng bộ nhớ
            client_delete(my_client)
